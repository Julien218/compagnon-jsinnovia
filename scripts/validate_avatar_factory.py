...
required = [
    ...
    ROOT / 'scripts/test_avatar_generic_subjects.py',
...
upload_source = (ROOT / 'scripts/avatar_reference_upload_server.py').read_text(encoding='utf-8')
...
assert cfg['capabilities']['generic_subjects'] is True
assert cfg['capabilities']['auto_manifest'] is True
assert cfg['capabilities']['single_view'] is True
assert cfg['capabilities']['four_view'] is True
assert set(cfg['capabilities']['subject_kinds']) >= {'auto', 'person', 'animal', 'bird', 'object', 'other'}
assert set(cfg['capabilities']['rig_modes']) >= {'auto', 'none', 'humanoid', 'quadruped', 'avian', 'generic'}
...
assert 'upsert_character_manifest' in upload_source
assert '/subjects/register' in upload_source
assert 'generic_subjects' in upload_source
assert 'default_rig_mode' in upload_source
...
assert 'normalized_rig_mode' in finalizer_source
assert 'job_metadata_from_database' in finalizer_source
assert 'rig_mode == "none"' in finalizer_source
assert 'static_asset' in finalizer_source
assert "'fragment_cleanup': fragment_cleanup" in finalizer_source or '"fragment_cleanup": fragment_cleanup' in finalizer_source

for script in [
    'avatar_factory_server.py',
    'avatar_runtime_qa.py',
    'test_avatar_runtime_qa.py',
    'test_avatar_generic_subjects.py',
    'avatar_reference_upload_server.py',
    'avatar_preview_server.py',
    'blender_finalize_avatar.py',
    'render_avatar_preview.py',
    'sync_finops.py',
]:
